...
import { inventoryAPI, purchasesAPI, suppliersAPI, productsAPI } from '../api'
...
const newProductForm = ref({
  product_name: '', category: '', brand: '新品候选', supplier_id: '', sku_name: '', specification: '标准规格',
  cost_price: 0, sale_price: 0, purchase_quantity: 100, purchase_price: 0,
  warehouse_name: '主仓', safety_stock: 30, warning_threshold: 50, expected_arrival_time: '', selling_points: '',
})
...
async function onProductChange() {
  form.value.sku_id = ''; form.value.purchase_price = 0; form.value.supplier_id = ''; productSkus.value = []
  if (!selectedProductId.value) return
  try { const { data } = await productsAPI.skus(selectedProductId.value); productSkus.value = data } catch {}
}
async function onSkuChange() {
  const sku = productSkus.value.find((s: any) => s.sku_id === form.value.sku_id)
  if (sku) { form.value.purchase_price = sku.cost_price || 0; form.value.supplier_id = sku.supplier_id || '' }
}
async function openCreate(suggestion?: any) {
  selectedProductId.value = ''; productSkus.value = []; form.value.expected_arrival_time = defaultArrival()
  if (suggestion) {
    const p = products.value.find((p: any) => p.product_name === suggestion.product_name)
    if (p) { selectedProductId.value = p.product_id; await onProductChange() }
    form.value.sku_id = suggestion.sku_id || ''
    form.value.purchase_quantity = suggestion.suggested_quantity || 0
    form.value.purchase_price = 0; form.value.supplier_id = suggestion.supplier_id || ''
    const sku = productSkus.value.find((s: any) => s.sku_id === form.value.sku_id)
    if (sku) { form.value.purchase_price = sku.cost_price || 0; form.value.supplier_id = form.value.supplier_id || sku.supplier_id || ''; if (!form.value.purchase_quantity) form.value.purchase_quantity = (sku.warning_threshold || 0) * 2 || 10 }
  } else {
  ...
  form.value.purchase_status = '待审核'; showCreate.value = true
}
async function saveCreate() {
  try { await purchasesAPI.create(form.value); showCreate.value = false; await load(); if (showAlerts.value) await fetchAlerts() }
  catch (e: any) { alert(e.response?.data?.message || '保存失败') }
}
function openNewProductPurchase() {
  newProductForm.value = {
    product_name: '', category: '', brand: '新品候选',
    supplier_id: suppliers.value[0]?.supplier_id || '', sku_name: '', specification: '标准规格',
    cost_price: 0, sale_price: 0, purchase_quantity: 100, purchase_price: 0,
    warehouse_name: '主仓', safety_stock: 30, warning_threshold: 50, expected_arrival_time: defaultArrival(), selling_points: '',
  }
  showNewProduct.value = true
}
async function saveNewProductPurchase() {
  try {
    const payload = { ...newProductForm.value, sku_name: newProductForm.value.sku_name || `${newProductForm.value.product_name} 默认SKU`, purchase_price: newProductForm.value.purchase_price || newProductForm.value.cost_price }
    await purchasesAPI.createNewProduct(payload); showNewProduct.value = false; await load()
  } catch (e: any) { alert(e.response?.data?.message || '新品采购保存失败') }
}
async function loadSuppliers() { const { data } = await suppliersAPI.list({ pageSize: 100 }); suppliers.value = data.data }
async function loadProducts() { const { data } = await productsAPI.list({ pageSize: 1000 }); products.value = data.data }
...
function goToPurchase(row: any) {
  openCreate({ sku_id: row.sku_id, product_name: row.product_name, suggested_quantity: row.suggested_quantity || 0, supplier_id: row.supplier_id || '' })
}
...
onMounted(() => { load(); loadSuppliers(); loadProducts() })
  ...
  <div class="page-body">
      ...
      <div class="modal" style="min-width:520px;padding-top:16px;">
        ...
        <div class="form-group">
          ...
          <select v-model="form.sku_id" class="form-select" @change="onSkuChange()" :disabled="!selectedProductId">
            ...
            <option v-for="s in productSkus" :key="s.sku_id" :value="s.sku_id">{{ s.sku_name }} · 成本 ¥{{ s.cost_price }}</option>
          </select>
        </div>
        <div class="form-group">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div class="form-group"><label class="form-label">采购数量</label><input v-model.number="form.purchase_quantity" type="number" class="form-input" /></div>
          <div class="form-group"><label class="form-label">采购价</label><input v-model.number="form.purchase_price" type="number" step="0.01" class="form-input" /></div>
        </div>
        <div class="form-group"><label class="form-label">总价</label><div class="total-price-display">¥{{ totalPrice.toLocaleString() }}</div></div>
        <div class="form-group"><label class="form-label">预计到货</label><input v-model="form.expected_arrival_time" type="date" class="form-input" /></div>
        <div class="form-actions">
    ...
    <div v-if="showNewProduct" class="modal-overlay" @click.self="showNewProduct = false">
      <div class="modal" style="min-width:680px;max-width:760px;padding-top:16px;">
        ...
        <div class="new-product-note">用于采购系统中尚不存在的新品。保存后会创建"待评估"商品、默认 SKU、主仓库存记录和待审核采购单。</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div class="form-group"><label class="form-label">新品名称</label><input v-model="newProductForm.product_name" class="form-input" /></div>
          <div class="form-group"><label class="form-label">品类</label><input v-model="newProductForm.category" class="form-input" placeholder="如 美妆 / 数码 / 食品饮料" /></div>
          <div class="form-group"><label class="form-label">品牌</label><input v-model="newProductForm.brand" class="form-input" /></div>
          ...
          <div class="form-group"><label class="form-label">SKU 名称</label><input v-model="newProductForm.sku_name" class="form-input" placeholder="为空时自动生成默认SKU" /></div>
          <div class="form-group"><label class="form-label">规格</label><input v-model="newProductForm.specification" class="form-input" /></div>
          <div class="form-group"><label class="form-label">成本价</label><input v-model.number="newProductForm.cost_price" type="number" step="0.01" class="form-input" @change="newProductForm.purchase_price = newProductForm.purchase_price || newProductForm.cost_price" /></div>
          <div class="form-group"><label class="form-label">建议售价</label><input v-model.number="newProductForm.sale_price" type="number" step="0.01" class="form-input" /></div>
          <div class="form-group"><label class="form-label">采购数量</label><input v-model.number="newProductForm.purchase_quantity" type="number" class="form-input" /></div>
          <div class="form-group"><label class="form-label">采购价</label><input v-model.number="newProductForm.purchase_price" type="number" step="0.01" class="form-input" /></div>
          <div class="form-group"><label class="form-label">安全库存</label><input v-model.number="newProductForm.safety_stock" type="number" class="form-input" /></div>
          <div class="form-group"><label class="form-label">预警阈值</label><input v-model.number="newProductForm.warning_threshold" type="number" class="form-input" /></div>
          <div class="form-group"><label class="form-label">仓库</label><input v-model="newProductForm.warehouse_name" class="form-input" /></div>
          <div class="form-group"><label class="form-label">预计到货</label><input v-model="newProductForm.expected_arrival_time" type="date" class="form-input" /></div>
        </div>
        <div class="form-group"><label class="form-label">卖点/采购理由</label><textarea v-model="newProductForm.selling_points" class="form-input" rows="3"></textarea></div>
        <div class="form-group"><label class="form-label">采购总价</label><div class="total-price-display">¥{{ newProductTotalPrice.toLocaleString() }}</div></div>
        <div class="form-actions">
...
.total-price-display {
  ...
  border: 1px solid var(--rule); font-family: var(--font-serif);
  font-size: 22px; font-weight: 900; color: var(--vermillion);
}

.new-product-note {
  border-left: 4px solid var(--vermillion); background: var(--paper-dark);
  padding: 12px; margin-bottom: 16px; color: var(--ink-soft);
